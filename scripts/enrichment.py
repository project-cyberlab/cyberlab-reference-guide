...
RESEARCHED_FLAGS: dict[str, dict] = {
    ...
    'PECmd': {
        ...
        '--csvf': 'An analyst would use the --csvf flag when generating a timeline view to extract and correlate chronological program execution timestamps from multiple Prefetch files during batch processing.',
        '-d': 'An analyst would use the -d flag when processing an entire prefetch directory to gather comprehensive execution context from all files, as demonstrated in the examples where the flag was used with the Prefetch folder path.',
    ...
    'RecentFileCacheParser': {
        '--json': 'An analyst would use the --json flag when they need to save the parsed RecentFileCache.bcf data in JSON format to a specified directory.',
    },
    'WxTCmd': {
    ...
    'diec': {
        '-t': 'When building the Docker container for Detect It Easy, an analyst would use the -t flag to tag the image with the specified name.',
    },
    'dumpcap': {
    ...
    'foremost': {
        ...
        '-o': 'An analyst would use the `-o` flag with `foremost` when they need to specify a custom output directory for recovered files instead of the default "output" directory.',
        '-q': 'An analyst would use the -q flag when performing quick file recovery by searching for file headers and footers on 512-byte boundaries to speed up the process.',
    ...
    'ils': {
        '-m': "An analyst would use the '-m' flag with 'fls' when gathering temporal data from file systems to create a timeline for forensic analysis.",
    },
    'mactime': {
        ...
        '-d': "An analyst would use the '-d' flag when they need to output the timeline data in comma-delimited format for importing into a spreadsheet to include as a table or for further analysis.",
        ...
        '-m': 'An analyst would use the -m flag with fls when gathering temporal data from file systems to create a body file for mactime to generate a timeline.',
    ...
    'olevba': {
        ...
        '-f': 'An analyst would use the -f flag when scanning password-protected zip files to specify the file pattern (e.g., "*.doc") to extract and analyze within the archive.',
    ...
    'pdf-parser': {
        '-f': 'When analyzing a malicious PDF to extract embedded content like JavaScript, an analyst would use the -f flag with pdf-parser.',
    },
    'pdf-parser.py': {
        '-f': 'When an analyst is processing a malicious PDF file that uses /Filter /ASCIIHexDecode /FlateDecode to extract or analyze embedded content.',
    },
    'pinfo.py': {
        '--sections': "An analyst would use the --sections flag with pinfo.py to verify which file paths were included in the timeline processing based on the filter file's specifications.",
    },
    'scalpel': {
        '-i': 'An analyst would use the -i flag when the names of disk images are listed in a separate file that needs to be read by scalpel.',
        '-q': 'An analyst would use the -q flag when generating or updating a carve coverage blockmap file to specify the cluster size for the blockmap.',
        '-t': 'An analyst would use the -t flag when generating or updating a carve coverage blockmap file.',
    },
}
