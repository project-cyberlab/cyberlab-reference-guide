...
| **Captured from** | `cyberlab-aio` via `--help` on 2026-08-11 — [raw help output](../../capture/cyberlab-aio/help/tcpflow.help.txt) |
...
| `-R` | file | read packets from tcpdump pcap file TO FINISH CONNECTIONS | An analyst would use the -R flag when processing a pcap file captured with tcpdump -w to rebuild the flows. |
...
| `-X` | — | filename : DFXML output to filename | An analyst would use the -X flag when generating a DFXML report to document every TCP connection and system details for forensic analysis. |
